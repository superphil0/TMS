...
namespace TMS
{
  public class DataLoader
  {
    ...
    public async static Task<List<Match>> LoadSchedule(List<Team> cachedTeams, Team team, string year)
    {
      ...
      {
        HttpClient httpClient = new HttpClient();
        HttpRequestMessage requestMessage = new HttpRequestMessage(HttpMethod.Get, "http://www.transfermarkt.com/bayern-munich/spielplan/verein/" + team.TeamId.ToString() + "/saison_id/" + year);
        requestMessage.Headers.Add("User-Agent", "Mozilla/5.0");
        ...
        if (st.Count > 0)
        {
          ...
          foreach (var row in rows)
          {
            ...
            string result = row.SelectNodes("td").ElementAt(9).InnerHtml;
